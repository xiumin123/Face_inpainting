...
from .utils import vis_parsing
from .utils import tensor_argmax
class Trainer():
...
###########add
        n_classes = 19
        #bsize = 8
        self.netP = net.BiSeNet(n_classes=n_classes).cuda()
        ...
        if args.distributed:
           ...
           ##add
            self.netP = DDP(self.netP, device_ids= [args.local_rank], output_device=[args.local_rank],find_unused_parameters=True)
    ...
    def load(self):
        try:
            #add
            pgath = './model_pr/model_pr/fp_256.pth'	
            self.netP.load_state_dict(torch.load(pgath, map_location='cuda'))
            self.netP.eval()
...
####add   
            ...
            with torch.no_grad():
                #print(images.size())
                feat_fuse = self.netP(images)#1,3,256,256 , out16, out32
                #print("out is:",out.size())
            #out1 = tensor_argmax(out)#1,1,256,256
            #print("out1 is:",out1.size())
            #out1 = out1.cuda()

            # in: [rgb(3) + edge(1)]
           # print("images_masked is:",images_masked.size())
           # print("masks is:",masks.size())
            pred_img = self.netG(images_masked, masks,feat_fuse)#feat_fuse
            ...
            if self.iteration % 1000==0:
               # with torch.no_grad(): 
                 # out_src= self.netP(images)  #[4,19,160,160]
                 # vis_im = vis_parsing(comp_img, out) #  completion image  comp_img/pred_img
                 # vis_x = vis_parsing(images, out_src) #input 
               imgs = torch.cat((
                    images.cpu(),
                    images_masked.cpu(),
                    #vis_x.cpu(),
                    #vis_im.cpu(),
                    comp_img.cpu()), dim=0)
               imgpath = os.path.join('../results','step%d.png' % idx)
