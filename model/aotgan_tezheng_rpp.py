...
class InpaintGenerator(BaseNetwork):
    def __init__(self, args):  # 1046
        ...
        self.spfm0  = SPFM(256,256,4)#256
        self.spfm1  = SPFM(256,256,4)
        self.spfm2  = SPFM(256,256,4)
        self.spfm3  = SPFM(256,256,4)
        self.spfm4  = SPFM(256,256,4)
        self.spfm5  = SPFM(256,256,4)
        self.spfm6  = SPFM(256,256,4)
        self.spfm7  = SPFM(256,256,4)
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Conv2d(64, 3, 3, stride=1, padding=1)#64
        )

        self.init_weights()

    def forward(self, x, mask, feat_fuse):
        x = torch.cat([x, mask], dim=1) #torch.cat([x, mask], dim=1)   x = torch.cat([x, mask, out1], dim=1)
        x = self.encoder(x)#[8, 256, 40, 40]
        #print('encoder',x.size())   
        #print('feat_fuse',feat_fuse.size())    #[8, 256, 20, 20]
        feat_fuse = F.interpolate(feat_fuse, scale_factor=2, mode="bilinear",align_corners=True)#[8, 256, 40, 40]
        #print('feat_fuse1',feat_fuse.size()) 
        x = torch.cat([x,feat_fuse],dim=1)#
        x = self.conv1d(x)#[8, 256, 40, 40]
        #print('cat',x.size())
        #x = self.middle(x)
       # x=x.ToTensor()
        #print(x.shape)  #8,256,40,40,
        ...
        x =  self.spfm5(x)
        x =  self.spfm6(x)
        x =  self.spfm7(x)#8,512 40,40
        x = self.decoder(x)#8, 3, 160, 160]
        x = torch.tanh(x)#8, 3, 160, 160]
